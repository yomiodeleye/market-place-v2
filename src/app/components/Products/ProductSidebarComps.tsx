...
function ProductSidebarComps({ productFilter }) {
  ...
  return (
    <div className='product-sidebar'>
      <div className='product-sidebar-widget mb-[25px] border-b border-[#dddddd] pb-[30px]'>
        ...
        <div className='flex flex-col items-start pt-[20px]'>
          {productFilter[0]?.categoryList?.map(singleCategoryList => (
            <button
              type='button'
              className={`${textHover} mb-[10px] capitalize last:mb-0`}
            ...
            <li className='mb-[10px]' key={item.id}>
              <label htmlFor={item.filterLabel} className={`${textHover}`}>
                <input
                  ...
                  id={item.filterLabel}
                  checked={
                    !!filter.filterData.find(data => data.key === item.checked)
                  }
                  onChange={data =>
                    filterChangeHandler(data.target.checked, {
                      title: item.name,
                      key: item.key,
                      group: item.group
                    })
      ...
      <div className='product-sidebar-widge mb-[25px] border-b border-[#dddddd] pb-[30px]'>
        ...
        >
          <div className='price-form-field mb-[15px]'>
            <label className='mb-[5px] flex' htmlFor='priceForm'>
              Form
            </label>
            <div className='flex h-[45px] items-center border border-[#dddddd] px-[10px]'>
              <span className='pr-[5px] text-[#777777]'>$</span>
              <input
                required
                ref={fromPriceRef}
                id='priceForm'
          ...
          <div className='price-form-field'>
            <label className='mb-[5px] flex' htmlFor='priceTo'>
              To
            </label>
            <div className='flex h-[45px] items-center border border-[#dddddd] px-[10px]'>
              <span className='pr-[5px] text-[#777777]'>$</span>
          ...
          <div className='price-btn-wrap pt-[20px]'>
            <button
              type='submit'
              className='hover:bg-primary flex h-[40px] items-center bg-black px-[25px] py-[8px] text-white transition-all'
            >
              Filter
            </button>
          </div>
